...
_.extend(Pool.prototype, {
        ...
        if (cb) {
            ...
            job.on('success', function (r) { 
                try {
                    ...
                    // util.debug("Details: " + util.inspect(job.options));
                }
            });
    ...
    _findFreeWorkerProcess: function () {
        for (pid in this.worker_processes) {
            ...
            if (worker_process.job || worker_process.is_terminal) {
                // Workers with jobs or shutting down are not free
                continue;
            }
        ...
        if (this.backlog.length) {
            ...
            process.nextTick(function () {
                for (pid in self.worker_processes) {
                    if (self.worker_processes[pid].job) {
                        return;
                    }
                }
                self.emit('drain');
...
function WorkerProcess (pool, options) {
    ...
    this._cprocess.on('message', function (msg) {
        ...
        if (self.is_terminal) {
            if (self.job) {self.job.worker_process = null; }
            self.job = null;
        }
    });
